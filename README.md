...
├── docs/
│   ├── API_설계서.md            # 프론트-백엔드 연동 30종 REST 통신 명세서
...
└── pages/
    ...
    ├── admin_class.html      # 관리자 전용 클래스 모니터링 및 최고 권한 대시보드
...
| 역할 | 주요 기능 |
|------|-----------| 
| **관리자** | 전체 유저 조회·역할 변경·삭제, 전체 클래스 조회·삭제, **클래스 상태 강제 모니터링 및 소유권(담당 교수) 즉시 이전 가능 (`admin_class.html`)** |
...
1. **API 아키텍처 (`api.js`, `api_mock.js`, `api_server.js`)**
   ...
   - **Real API**: `api_server.js` 기반 `fetch` 프로토콜 골격 제공 (현재 30종의 Front API 호출 구조와 100% 1:1 매핑 연동 구현 확보)
