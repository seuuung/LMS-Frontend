...
특히, 서버 응답이 성공(`success: true`)하더라도 `data` 필드가 누락되거나 `undefined`인 경우, 기존 클라이언트 상태를 유지할 수 있도록 전체 응답 객체를 반환하거나 빈 객체를 반환하는 **방어적 응답 처리**를 수행하여 프론트엔드 상태의 무결성을 보장합니다.
...
### 👤 사용자 역할 (Role) 정의
모든 유저 데이터 및 로그인 응답에 포함되는 `role` 필드는 아래 3가지 문자열 중 하나를 가집니다.

| Role 값 (String) | 역할 설명 | 시스템 영향 |
| :--- | :--- | :--- |
| `admin` | 최고 관리자 | 모든 데이터 조회 및 관리 권한, `/admin`으로 리다이렉션 |
| `prof` | 교수자 / 강사 | 본인 클래스 관리 및 성적 확인, `/professor`로 리다이렉션 |
| `student` | 학생 / 학습자 | 강좌 수강 및 질문 등록, `/student`로 리다이렉션 |

> **주의**: 모든 `role` 값은 반드시 **소문자**로 전달하며, 프론트엔드 라우팅 로직의 핵심 키로 사용됩니다.
